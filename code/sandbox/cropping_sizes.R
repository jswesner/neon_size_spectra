...
# What proportion of the data is fish vs inverts? -------------------------
...
dat_both = bind_rows(dat_fish, dat_invert)

dat_both %>% 
  mutate(mgm2 = dw*no_m2) %>% 
  group_by(animal_type) %>% 
  summarize(mgm2_sum = sum(mgm2)) %>% 
  ungroup %>% 
  mutate(total = sum(mgm2_sum),
         prop = mgm2_sum/total)
...
# precompile code
fit_model = stan_model("models/stan_spectra_singlesample.stan")
...
b = c(-2, -1.6, -1.3)
b_true = b
crops = c(0, 0.2, 0.3, 0.5, 0.9) # get the whole data set or the lower or upper 50%, 10%, etc
rep = 1:2
...
saveRDS(parameter_recovery_withcounts, file = "models/posteriors/parameter_recovery_withcounts.rds")

parameter_recovery_withcounts = readRDS(file = "models/posteriors/parameter_recovery_withcounts.rds") %>% 
  mutate(crop_method = case_when(sizes == "largest quantiles" ~ "Only Fish", 
                                 TRUE ~ "Only Inverts"),
         NEON_proportions = case_when(sizes == "largest quantiles" ~ 0.1,
                                      TRUE ~ 0.9))
...
parameter_recovery_withcounts %>% 
  ...
  geom_hline(aes(yintercept = true_lambda), linetype = "dotted") + 
  geom_vline(aes(xintercept = NEON_proportions), linetype = "dashed") +
  facet_grid(true_lambda ~ crop_method) +
  # guides(color = "none") +
  labs(title = "Analyzing only fish leads to bad estimates",
