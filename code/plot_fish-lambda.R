...
neon_sizes_2016_2021 = readRDS(file = "data/derived_data/fish_dw-wrangled.rds") %>% 
  ...
  group_by(site_id) %>% 
  mutate(site_int = cur_group_id()) %>% 
  group_by(year) %>% 
  mutate(year_int = cur_group_id()) %>% 
  ungroup
...
fishmod = readRDS("models/stan_fishonly_gppxtemp2023-02-18.rds")
...
posts_long = as_draws_df(fishmod) %>% as_tibble() %>% 
  mutate(iter = 1:nrow(.)) %>% 
  filter(iter <= 500) %>% 
...
fish_dw %>% filter(site_id == "ARIK") %>% View()
