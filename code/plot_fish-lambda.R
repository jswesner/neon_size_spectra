...
qlog_om_s = quantile(unique(dat_fish$log_om_s), probs = c(0.25, 0.5, 0.75), na.rm = T) %>% 
  as_tibble() %>% 
  pivot_longer(cols = everything(), values_to = "log_om_s", names_to = "quantile_om") %>% 
  mutate(quantile_om = c("Low OM", "Median OM", "High OM"))

qlog_gpp_s = quantile(unique(dat_fish$log_gpp_s), probs = c(0.25, 0.5, 0.75), na.rm = T) %>% 
  as_tibble() %>% 
  pivot_longer(cols = everything(), values_to = "log_gpp_s", names_to = "quantile_gpp") %>% 
  mutate(quantile_gpp = c("Low GPP", "Median GPP", "High GPP"))

# lambda regression
post_lines = as_draws_df(fishmod) %>% as_tibble() %>% 
  expand_grid(mat_s = seq(min(dat_fish$mat_s), max(dat_fish$mat_s), length.out = 10)) %>% 
...
ggsave(isd_temp_by_gpp, file = "plots/isd_temp_by_gpp_om-fishonly.jpg", width = 6, height = 6, units = "in")
saveRDS(isd_temp_by_gpp, file = "plots/isd_temp_by_gpp_om-fishonly.rds")
...
ggsave(isd_by_temp, file = "plots/isd_by_temp-fishonly.jpg", width = 5, height = 5, units = "in")
saveRDS(isd_by_temp, file = "plots/isd_by_temp-fishonly.rds")
...
# lambda regression
post_lines_xis_gpp = as_draws_df(fishmod) %>% as_tibble() %>% 
  expand_grid(log_gpp_s = seq(min(dat_fish$log_gpp_s), max(dat_fish$log_gpp_s), length.out = 10)) %>% 
  expand_grid(mat_s = quantile(unique(dat_fish$mat_s), probs = c(0.25, 0.5, 0.75))) %>% 
...
dat_toplot = dat_sims %>% 
  ...
  left_join(dat_fish %>% ungroup %>% distinct(site_id, mean, log_gpp_s, log_om_s))
...
isd_per_sample_plot
saveRDS(isd_per_sample_plot, file = "plots/isd_per_sample_plot-fishonly.rds")
ggsave(isd_per_sample_plot, file = "plots/isd_per_sample_plot-fishonly.jpg", 
       width = 6, height = 6)
...
ggsave(isd_lines_onepanel, file = "plots/isd_lines_onepanel-fishonly.jpg", width = 5.5, height = 5, units = "in", dpi = 500)
saveRDS(isd_lines_onepanel, file = "plots/isd_lines_onepanel-fishonly.rds")
