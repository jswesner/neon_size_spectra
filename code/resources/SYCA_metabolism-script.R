...
# debugonce(clean_DO)
clean_DO(siteCode ='SYCA', return = FALSE) 
# debugonce(clean_temp)
clean_temp(siteCode = 'SYCA', return = FALSE)
SYCA_clean_temp = readRDS(file = "./ignore/site-gpp-data/SYCA_clean_temp.rds")
SYCA_clean_DO =
  readRDS("./ignore/site-gpp-data/SYCA_clean_DO.rds")%>% dplyr::filter(lubridate::year(timePeriod) != 2021 ) 

plot(ts(SYCA_clean_DO[,grep('DO', names(SYCA_clean_DO))])) 
plot(ts(SYCA_clean_temp$temp_102))

rstudioapi::jobRunScript(
  path = "./ignore/metab-models/SYCA_doLM.R",
  name = "SYCA LM",
  workingDir = getwd(),
  importEnv = FALSE,
  exportEnv = FALSE
...
rstudioapi::jobRunScript(
  path = "./ignore/metab-models/SYCA_tempLM.R",
  name = "SYCA tempLM",
  workingDir = getwd(),
  importEnv = FALSE,
  exportEnv = FALSE
...
SYCA_tempLM = readRDS(file = "./ignore/metab-models/SYCA_tempLM.rds")
SYCA_clean_temp = SYCA_clean_temp %>%
  cbind(.,predict(SYCA_tempLM,
                  .,
                  se.fit = FALSE,
                  type = 'response'))

rstudioapi::jobRunScript(
  path = "./ignore/metab-models/SYCA_metModel.R",
  name = "SYCA metMM",
  workingDir = getwd(),
  importEnv = FALSE,
  exportEnv = FALSE
...
SYCA_full_mle = readRDS("./ignore/metab-models/SYCA_full_mle.rds")


SYCA_mle_params = get_params(SYCA_full_mle, uncertainty = 'ci') %>%
...
plot_met_series(SYCA_met_full)
# remove some data points above 25 which are anomolous
debugonce(clean_met_data)
SYCA_met_clean = clean_met_data(SYCA_met_full, doCutOff = 15)
...
SYCA_met_clean %>%
  ...
  saveRDS("./data/derived_data/clean-met-files/SYCA2019_met.rds")
SYCA_met_clean %>%
  dplyr::filter(lubridate::year(solar.time) == 2020)%>%
  saveRDS("./data/derived_data/clean-met-files/SYCA2020_met.rds")

SYCA_met_clean %>%
  dplyr::filter(lubridate::year(solar.time) == 2021)%>%
  saveRDS("./data/derived_data/clean-met-files/SYCA2021_met.rds")

SYCA_met_clean %>%
  dplyr::filter(lubridate::year(solar.time) == 2022)%>%
  saveRDS("./data/derived_data/clean-met-files/SYCA2022_met.rds")
...
SYCA_met %>% 
  ...
  saveRDS("./data/derived/clean-met-files/SYCA_met.rds")
