...
# clean and view DO
COMO_DO = clean_DO(siteCode ='COMO', return = TRUE, save = FALSE);names(COMO_DO)
COMO_DO_xts = xts(COMO_DO[,grep('DO',names(COMO_DO))], order.by= COMO_DO$timePeriod)
dygraph(COMO_DO_xts, main = "COMO DOs") %>% dyRangeSelector()

COMObadDates = c(seq(as.Date("2020-06-08"),as.Date("2020-07-01"), by = 1),
                 seq(as.Date("2021-08-24"),as.Date("2021-10-06"), by = 1))

COMO101dates = c(seq(as.Date("2020-02-08"),as.Date("2020-02-14"), by = 1),
                 seq(as.Date("2020-12-10"),as.Date("2021-03-28"), by = 1),
                 seq(as.Date("2017-12-09"),as.Date("2017-12-29"), by = 1))
COMO_DO %>% 
  dplyr::mutate(DO_102 = case_when(as.Date(timePeriod) %in% COMO101dates ~ DO_101,
                                   as.Date(timePeriod) %in% COMObadDates ~ NA_real_,
                                   TRUE ~ DO_102)) %>% 
  saveRDS(file = here::here("ignore/site-gpp-data/COMO_clean_DO.rds"))

# temperature profiles look good here.
COMO_temp = clean_temp(siteCode = 'COMO', return = TRUE, save = TRUE)
COMO_temp_xts = xts(COMO_temp[,grep('temp',names(COMO_temp))], order.by= COMO_temp$timePeriod)
...
COMO_met = clean_met_data(COMO_met)
...
COMO_met %>% 
  ...
  saveRDS("./ignore/site-gpp-data/clean-met-files/COMO_met.rds")


# COMO test 
tz(COMO_met$solar.time)
...
# tictoc::tic();
mm_bin <- metab(bayes_specs, data=COMO_test)#;tictoc::toc()
