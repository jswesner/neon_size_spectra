# MART metabolism script
rm(list = ls())
source("./code/resources/01_load-packages.R")
# debugonce(clean_DO)
MART_DO = clean_DO(siteCode ='MART', save = FALSE, return = TRUE);names(MART_DO)
MART_DO_xts = xts(MART_DO[,grep("DO_*",names(MART_DO))], order.by = MART_DO$timePeriod)
dygraph(MART_DO_xts, main = "MART DOs") %>% dyRangeSelector()

MART101badDates = c(seq(as.Date("2018-03-29"), as.Date("2018-04-08"), by = 1),
                    seq(as.Date("2020-01-20"), as.Date("2020-02-19"), by = 1),
                    seq(as.Date("2020-09-12"), as.Date("2020-09-19"), by = 1))
MART102badDates = c(seq(as.Date("2020-06-22"), as.Date("2020-07-07"), by = 1))
MART111badDates = c(seq(as.Date("2021-06-22"), as.Date("2021-07-19"), by = 1))

MART_DO = MART_DO %>% 
  dplyr::mutate(DO_101 = ifelse(as.Date(timePeriod) %in% MART101badDates, NA_real_, DO_101),
                DO_102 = ifelse(as.Date(timePeriod) %in% MART102badDates, NA_real_, DO_102),
                DO_111 = ifelse(as.Date(timePeriod) %in% MART111badDates, NA_real_, DO_111))

lm101to102= lm(DO_102 ~ DO_101, data = MART_DO);summary(lm101to102);plot(DO_102 ~ DO_101, data = MART_DO)
lm111to112 = lm(DO_112 ~ DO_111, data = MART_DO);summary(lm111to112);plot(DO_112 ~ DO_111, data = MART_DO)

# fill with predicted values
MART_DO$pred1 = predict(lm101to102, newdata = data.frame(DO_101 = MART_DO$DO_101))
MART_DO$pred2 = predict(lm111to112, newdata = data.frame(DO_111 = MART_DO$DO_111))

MART_clean_DO = MART_DO %>%
  dplyr::mutate(DO.obs = case_when(is.na(DO_112) & is.na(DO_111) & !is.na(DO_102) ~ DO_102,
                                   is.na(DO_112) & is.na(DO_111) & is.na(DO_102) & !is.na(DO_101) ~ pred1,
                                   is.na(DO_112) & is.na(DO_102) & !is.na(DO_111) ~ pred2,
                                   is.na(DO_112) & is.na(DO_102) & is.na(DO_101) & is.na(DO_111) ~ NA_real_,
                                   TRUE ~ DO_112)) %>%
  dplyr::select(timePeriod, DO_102 = "DO.obs", hour)
plot(ts(MART_clean_DO$DO_102))

saveRDS(MART_clean_DO, file = here::here("ignore/site-gpp-data/MART_clean_DO.rds"))
rm(list = ls())
MART_temp = clean_temp(siteCode = 'MART', return = TRUE, save = FALSE)
MART_temp_xts = xts(MART_temp[,grep("temp_*",names(MART_temp))], order.by = MART_temp$timePeriod)
dygraph(MART_temp_xts, main = "MART temps") %>% dyRangeSelector()

MART101badDates = c(seq(as.Date("2018-02-01"), as.Date("2018-02-24"), by = 1),
                    seq(as.Date("2019-06-10"), as.Date("2020-07-29"), by = 1))

MART102badDates = c(seq(as.Date("2021-01-12"), as.Date("2021-01-19"), by = 1))

MART111badDates = c(seq(as.Date("2022-06-21"), as.Date("2022-08-03"), by = 1))

MART_temp = MART_temp %>% 
  dplyr::mutate(temp_101 = ifelse(as.Date(timePeriod) %in% MART101badDates, NA_real_, temp_101),
                temp_102 = ifelse(as.Date(timePeriod) %in% MART102badDates, NA_real_, temp_102),
                temp_111 = ifelse(as.Date(timePeriod) %in% MART111badDates, NA_real_, temp_111))

lm101to102= lm(temp_102 ~ temp_101,data = MART_temp);summary(lm101to102);plot(temp_102 ~ temp_101, data = MART_temp)
lm111to112 = lm(temp_112 ~ temp_111,data = MART_temp);summary(lm111to112);plot(temp_112 ~ temp_111,data = MART_temp)

# fill with predicted values
MART_temp$pred1 = predict(lm101to102, newdata = data.frame(temp_101 = MART_temp$temp_101))
MART_temp$pred2 = predict(lm111to112, newdata = data.frame(temp_111 = MART_temp$temp_111))

MART_clean_temp = MART_temp %>%
  dplyr::mutate(temp.obs = case_when(is.na(temp_112) & is.na(temp_111) & !is.na(temp_102) ~ temp_102,
                                     is.na(temp_112) & is.na(temp_111) & is.na(temp_102) & !is.na(temp_101) ~ pred1,
                                     is.na(temp_112) & !is.na(temp_111) & is.na(temp_102) & is.na(temp_101) ~ pred2,
                                     is.na(temp_112)& is.na(temp_111)& is.na(temp_102) & is.na(temp_101) ~ NA_real_,
                                     TRUE ~ temp_112)) %>%
  dplyr::select(timePeriod, temp_102 = "temp.obs", hour)
plot(ts(MART_clean_temp$temp_102))

saveRDS(MART_clean_temp, file = here::here("ignore/site-gpp-data/MART_clean_temp.rds"))

## 
MART_clean_DO = readRDS(file = "./ignore/site-gpp-data/MART_clean_DO.rds")
MART_clean_temp = readRDS(file = "./ignore/site-gpp-data/MART_clean_temp.rds")

# debugonce(get_site_data)
MART_met = get_site_data(siteCode = "MART")

# remove some data points above 25 which are anomolous
# debugonce(clean_met_data)
MART_met_clean = clean_met_data(MART_met)
# Quick plot to check out the data series
# tz = attr(MART_met$solar.time,"tzone")

MART_met_clean %>% 
  # dplyr::filter(!is.na(solar.time)) %>%
  # dplyr::filter(!as.logical(outQF)) %>%
  # dplyr::filter(lubridate::year(solar.time) %in% c(2018,2019)) %>%
  ggplot()+
  geom_line(aes(x = solar.time, y = DO.obs, color = as.logical(outQF)))+
  theme_minimal()
plot_site("MART")

MART_met %>% 
  dplyr::filter(!is.na(solar.time)) %>%
  dplyr::filter(!as.logical(outQF)) %>%
  dplyr::filter(lubridate::year(solar.time) %in% c(2019,2020)) %>%
  saveRDS("./ignore/site-gpp-data/clean-met-files/MART_met.rds")


MART_met_clean %>%
  dplyr::filter(!as.logical(outQF)) %>%
  dplyr::filter(lubridate::year(solar.time) == 2018) %>%
  saveRDS("./data/derived_data/clean-met-files/MART2018_met.rds")
MART_met_clean %>%
  dplyr::filter(!as.logical(outQF)) %>%
  dplyr::filter(lubridate::year(solar.time) == 2019) %>%
  saveRDS("./data/derived_data/clean-met-files/MART2019_met.rds")
MART_met_clean %>%
  dplyr::filter(!as.logical(outQF)) %>%
  dplyr::filter(lubridate::year(solar.time) == 2020)%>%
  saveRDS("./data/derived_data/clean-met-files/MART2020_met.rds")
MART_met_clean %>%
  dplyr::filter(!as.logical(outQF)) %>%
  dplyr::filter(lubridate::year(solar.time) == 2021)%>%
  saveRDS("./data/derived_data/clean-met-files/MART2021_met.rds")
MART_met_clean %>%
  dplyr::filter(!as.logical(outQF)) %>%
  dplyr::filter(lubridate::year(solar.time) == 2022)%>%
  saveRDS("./data/derived_data/clean-met-files/MART2022_met.rds")


MART_test = MART_met %>%
  dplyr::filter(between(solar.time, as.POSIXct("2017-10-03 00:00:00"), as.POSIXct("2017-10-15 04:00:00")))

MART_test %>%
 
## run a quick version of the 
bayes_name <- mm_name(type='bayes', pool_K600='binned', err_obs_iid=TRUE,
                      err_proc_iid=FALSE, err_proc_GPP = TRUE, ode_method = "trapezoid")
bayes_specs <- specs(bayes_name)
# bayes_specs
# Model specifications:
#   model_name                 b_Kn_oipi_tr_plrckm.stan                                                         
# engine                     stan                                                                             
# split_dates                FALSE                                                                            
# keep_mcmcs                 TRUE                                                                             
# keep_mcmc_data             TRUE                                                                             
# day_start                  4                                                                                
# day_end                    28                                                                               
# day_tests                  full_day, even_timesteps, complete_data, pos_discharge, pos_depth                
# required_timestep          NA                                                                               
# GPP_daily_mu               3.1                                                                              
# GPP_daily_lower            -Inf                                                                             
# GPP_daily_sigma            6                                                                                
# ER_daily_mu                -7.1                                                                             
# ER_daily_upper             Inf                                                                              
# ER_daily_sigma             7.1                                                                              
# K600_daily_meanlog_meanlog 2.484906649788                                                                   
# K600_daily_meanlog_sdlog   1.32                                                                             
# K600_daily_sdlog_sigma     0.05                                                                             
# err_obs_iid_sigma_scale    0.03                                                                             
# err_proc_iid_sigma_scale   5                                                                                
# params_in                  GPP_daily_mu, GPP_daily_lower, GPP_daily_sigma, ER_daily_mu, ER_daily_upper, E...
# params_out                 GPP, ER, DO_R2, GPP_daily, ER_daily, K600_daily, K600_daily_predlog, K600_dail...
# n_chains                   4                                                                                
# n_cores                    4                                                                                
# burnin_steps               500                                                                              
# saved_steps                500                                                                              
# thin_steps                 1                                                                                
# verbose                    TRUE

# revise the specs for a test run 
bayes_specs <- revise(bayes_specs, day_start = 0, day_end = 24, burnin_steps= 10000, saved_steps= 5000,
                      thin_steps = 10,n_cores=5, n_chains = 3, GPP_daily_mu = 3, GPP_daily_sigma=2, verbose = TRUE)

# tictoc::tic();
mm_bin <- metab(bayes_specs, data=MART_test)#;tictoc::toc()

mm_bin

plot_metab_preds(mm_bin)

# get_params(mm)

plot_DO_preds(mm_bin)
predict_metab(mm)

x = predict_metab(mm_bin)

x %>% ggplot()+geom_point(aes(x = GPP, y = ER))