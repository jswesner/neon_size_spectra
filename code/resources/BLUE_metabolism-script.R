...
# debugonce(clean_DO)
clean_DO(siteCode ='BLUE', return = FALSE)
# debugonce(clean_temp)
clean_temp(siteCode = 'BLUE', return = FALSE)
...
plot(ts(BLUE_clean_DO[,grep('DO', names(BLUE_clean_DO))])) 
plot(ts(BLUE_clean_temp$temp_102))
rstudioapi::jobRunScript(
  path = "./ignore/metab-models/BLUE_doLM.R",
  name = "BLUE LM",
  workingDir = getwd(),
  importEnv = FALSE,
  exportEnv = FALSE
...
rstudioapi::jobRunScript(
  path = "./ignore/metab-models/BLUE_tempLM.R",
  name = "BLUE tempLM",
  workingDir = getwd(),
  importEnv = FALSE,
  exportEnv = FALSE
...
BLUE_tempLM = readRDS(file = "./ignore/metab-models/BLUE_tempLM.rds")
BLUE_clean_temp = BLUE_clean_temp %>%
  cbind(.,predict(BLUE_tempLM,
                  .,
                  se.fit = FALSE,
                  type = 'response'))
...
BLUE_full_mle = readRDS("./ignore/metab-models/BLUE_full_mle.rds")


BLUE_mle_params = get_params(BLUE_full_mle, uncertainty = 'ci') %>%
...
plot_met_series(BLUE_met_full)
# remove some data points above 25 which are anomolous
debugonce(clean_met_data)
BLUE_met_clean = clean_met_data(BLUE_met_full, doCutOff = 15)
...
BLUE_met_clean %>%
  ...
  saveRDS("./data/derived_data/clean-met-files/BLUE2019_met.rds")
BLUE_met_clean %>%
  dplyr::filter(lubridate::year(solar.time) == 2020)%>%
  saveRDS("./data/derived_data/clean-met-files/BLUE2020_met.rds")

BLUE_met_clean %>%
  dplyr::filter(lubridate::year(solar.time) == 2021)%>%
  saveRDS("./data/derived_data/clean-met-files/BLUE2021_met.rds")

BLUE_met_clean %>%
  dplyr::filter(lubridate::year(solar.time) == 2022)%>%
  saveRDS("./data/derived_data/clean-met-files/BLUE2022_met.rds")
...
BLUE_met %>% 
  ...
  saveRDS("./data/derived/clean-met-files/BLUE_met.rds")
