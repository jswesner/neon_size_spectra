rm(list = ls())
here::i_am("code/resources/metModelsMLE/LECO_metModel.R")
source(here::here("code/resources/01_load-packages.R"))
rerun = FALSE
if(rerun){
# debugonce(run_metab_multimodel)
tictoc::tic();run_metab_multimodel(siteCode = "LECO", metDf = NULL, return = FALSE, save = TRUE, parallel = FALSE);tictoc::toc()
}
siteCode = 'LECO'
load(file = here::here("ignore/metab-models/mleModLists/LECOmlemods.Rdata"))
...
# 
# saveRDS(mm3_sat, "./ignore/metab-models/LECO_full_mle.rds")

# source("./code/resources/01_load-packages.R")
# LECO_met_full = get_site_data(siteCode = "LECO") %>%
#   dplyr::select(-DO.pctsat)
# 
# discharge.daily = LECO_met_full %>%
...
# mm1 <- metab_mle(mle_specs, data = LECO_met_full)
# 
...
#                           day_start = -1, day_end = 23), data_daily = k600_mm1%>% select(-GPP.daily))
# km4 <- metab_night(data = LECO_met_full)
# # 
...
# 
# mm5 <- metab_mle(mle_specs, data = LECO_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
# ## 
# mle_specs_sat <- specs(mm_name(type = "mle", GPP_fun = 'satlight'))
# mm1_sat <- metab_mle(mle_specs_sat, data = LECO_met_full)
# 
...
#   facet_wrap(~model, scales = 'free_y')
# # 
# mm2_sat <- metab_mle(mle_specs_sat, data = LECO_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))
# 
# mm3_sat <- metab_mle(mle_specs_sat, data = LECO_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))
# 
# mm4_sat <- metab_mle(mle_specs_sat, data = LECO_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))
# 
# mm5_sat <- metab_mle(mle_specs_sat, data = LECO_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
# 
# ## satq10temp
# mle_specs_satq10 <- specs(mm_name(type = "mle", GPP_fun = 'satlight', ER_fun = 'q10temp'))
# mm1_satq10 <- metab_mle(mle_specs_satq10, data = LECO_met_full)
# 
# # identify negative values
...
#   facet_wrap(~model, scales = 'free_y')
# # 
# mm2_satq10 <- metab_mle(mle_specs_satq10, data = LECO_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))
# 
# mm3_satq10 <- metab_mle(mle_specs_satq10, data = LECO_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))
# 
# mm4_satq10 <- metab_mle(mle_specs_satq10, data = LECO_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))
# 
# mm5_satq10 <- metab_mle(mle_specs_satq10, data = LECO_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
# ####
# ####
# modList = ls()[grep("^mm\\d{1}.*", ls())] %>% purrr::map(~eval(as.symbol(.x)))
# save(modList, file = "./ignore/metab-models/mleModLists/LECOmlemods.rds")

# model assessment ----
# rm(list = ls())
# source("./code/resources/01_load-packages.R")
# load(file = "./ignore/metab-models/mleModLists/LECOmlemods.Rdata")
# modList_sub = unlist(lapply(modList, function(x) !is.data.frame(x)))
# modList = modList[modList_sub]
# 
...
# 
# saveRDS(mm2, "./ignore/metab-models/LECO_full_mle.rds")
...
# metab_fun(specs = bayes_specs, data = LECO_met_mod, data_daily = data_daily, info = info)
# 
# 
# 
# 
...
#   data = LECO_met_mod,    # named list of data
...
# # 
# debugonce(metab)
# mm_bin <- metab(bayes_specs, data=LECO_met_mod)
# 
# saveRDS(mm_bin, "./ignore/metab-models/LECO_test.rds")