rm(list = ls())
here::i_am("code/resources/metModelsMLE/HOPB_metModel.R")
source(here::here("code/resources/01_load-packages.R"))
# 
rerun = FALSE
if(rerun){
tictoc::tic();run_metab_multimodel(siteCode = "HOPB", metDf = NULL, return = FALSE, save = TRUE, parallel = FALSE);tictoc::toc()
}
siteCode = 'HOPB'
load(file = here::here("ignore/metab-models/mleModLists/HOPBmlemods.Rdata"))
...
# # Visual model assessment of top models
...
# gppMeanList = purrr::map(modList, ~calc_gpp_mean(.x, scaler = "*0.365"))
# 
# mods = data.frame(
...
# 
# # get clean data example
# HOPB_mod = HOPB_met_full %>% 
#   dplyr::filter(as.Date(solar.time) %in% seq(as.Date("2019-05-08"), as.Date("2019-05-30"), by = 1))
# 
# HOPB_mod = HOPB_mod %>% 
#   dplyr::mutate(DOmod = zoo::na.spline(DO.obs))
# 
# HOPB_mod %>%
#   ggplot()+
#   geom_point(aes(x = solar.time, y = DO.obs))+
#   geom_line(aes( x = solar.time, y = DOmod))+
#   geom_smooth(aes(x = solar.time, y = DO.obs, span = 0.1), se = FALSE)
# 
# badDates = 
# 
# HOPB_met_full = get_site_data(siteCode = "HOPB") %>%
...
# 
# tictoc::tic();run_metab_multimodel(siteCode = "HOPB", metDf = HOPB_met_full, return = FALSE, save = TRUE, parallel = FALSE);tictoc::toc()
