rm(list = ls())
here::i_am("code/resources/metModelsMLE/WALK_metModel.R")
source(here::here("code/resources/01_load-packages.R"))
# 
rerun = FALSE
if(rerun){
tictoc::tic();run_metab_multimodel(siteCode = "WALK", metDf = NULL, return = FALSE, save = TRUE, parallel = FALSE);tictoc::toc()
}
siteCode = 'WALK'
load(file = here::here("ignore/metab-models/mleModLists/WALKmlemods.Rdata"))
...
# 
# saveRDS(mm4, "./ignore/metab-models/WALK_full_mle.rds")

# WALK_met_full = get_site_data(siteCode = "WALK") %>%
#   dplyr::select(-DO.pctsat)
# 
# discharge.daily = WALK_met_full %>%
...
#                           day_start = -1, day_end = 23), data_daily = k600_mm1)
# km4 <- metab_night(data = WALK_met_full)
# # 
...
# # 
# mm2 <- metab_mle(mle_specs, data = WALK_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))
# 
# mm3 <- metab_mle(mle_specs, data = WALK_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))
# 
# mm4 <- metab_mle(mle_specs, data = WALK_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))
# 
# mm5 <- metab_mle(mle_specs, data = WALK_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
# ## 
# mle_specs_sat <- specs(mm_name(type = "mle", GPP_fun = 'satlight'))
# mm1_sat <- metab_mle(mle_specs_sat, data = WALK_met_full)
# 
...
#   facet_wrap(~model, scales = 'free_y')
# # 
# mm2_sat <- metab_mle(mle_specs_sat, data = WALK_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))
# 
# mm3_sat <- metab_mle(mle_specs_sat, data = WALK_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))
# 
# mm4_sat <- metab_mle(mle_specs_sat, data = WALK_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))
# 
# mm5_sat <- metab_mle(mle_specs_sat, data = WALK_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
# 
# ## satq10temp
# mle_specs_satq10 <- specs(mm_name(type = "mle", GPP_fun = 'satlight', ER_fun = 'q10temp'))
# mm1_satq10 <- metab_mle(mle_specs_satq10, data = WALK_met_full)
# 
# # identify negative values
...
#   facet_wrap(~model, scales = 'free_y')
# # 
# mm2_satq10 <- metab_mle(mle_specs_satq10, data = WALK_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))
# 
# mm3_satq10 <- metab_mle(mle_specs_satq10, data = WALK_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))
# 
# mm4_satq10 <- metab_mle(mle_specs_satq10, data = WALK_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))
# 
# mm5_satq10 <- metab_mle(mle_specs_satq10, data = WALK_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
# ####
# 
# # model assessment
# mods = data.frame(
...
# save(modList, file = "./ignore/metab-models/mleModLists/WALKmlemods.Rdata")
# 
# knitr::kable(mods)
# 
# # topMod = pick_model(mods)
# 
# # saveRDS(mm4_sat, "./ignore/metab-models/WALK_full_mle.rds")
# ###
...
# metab_fun(specs = bayes_specs, data = WALK_met_mod, data_daily = data_daily, info = info)
# 
# 
# 
# 
...
#   data = WALK_met_mod,    # named list of data
...
# # 
# debugonce(metab)
# mm_bin <- metab(bayes_specs, data=WALK_met_mod)
# 
# saveRDS(mm_bin, "./ignore/metab-models/WALK_test.rds")