# have to check on KO2~K600 conversion as temperatures -> 50C
#need to rerun models creations
rm(list = ls())
here::i_am("code/resources/metModelsMLE/SYCA_metModel.R")
source(here::here("code/resources/01_load-packages.R"))

rerun = FALSE
if(rerun){
SYCA_badDates = readRDS(here::here("ignore/site-gpp-data/SYCA_highGPPDates.rds"))
  
tictoc::tic();run_metab_multimodel(siteCode = "SYCA", metDf = NULL, return = FALSE, save = TRUE, parallel = FALSE, badDates = SYCA_badDates);tictoc::toc()
}
siteCode = 'SYCA'
load(file = here::here("ignore/metab-models/mleModLists/SYCAmlemods.Rdata"))
...
# 
# knitr::kable(mods, format = 'pipe')
# debugonce(slim_models)
# slim_models(mods)
# modsDf= find_best_fit(mods)
## Here is the final model choice after assessing the model fits
# 
# saveRDS(mm3_sat, "./ignore/metab-models/SYCA_full_mle.rds")



# SYCA_met_full = get_site_data(siteCode = "SYCA") %>%
#   dplyr::select(-DO.pctsat)
# 
# discharge.daily = SYCA_met_full %>%
...
#                           day_start = -1, day_end = 23), data_daily = k600_mm1)
# km4 <- metab_night(data = SYCA_met_full)
# # 
...
# # 
# mm2 <- metab_mle(mle_specs, data = SYCA_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))
# 
# mm3 <- metab_mle(mle_specs, data = SYCA_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))
# 
# mm4 <- metab_mle(mle_specs, data = SYCA_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))
# 
# mm5 <- metab_mle(mle_specs, data = SYCA_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
# ## 
# mle_specs_sat <- specs(mm_name(type = "mle", GPP_fun = 'satlight'))
# mm1_sat <- metab_mle(mle_specs_sat, data = SYCA_met_full)
# 
...
#   facet_wrap(~model, scales = 'free_y')
# # 
# mm2_sat <- metab_mle(mle_specs_sat, data = SYCA_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))
# 
# mm3_sat <- metab_mle(mle_specs_sat, data = SYCA_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))
# 
# mm4_sat <- metab_mle(mle_specs_sat, data = SYCA_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))
# 
# mm5_sat <- metab_mle(mle_specs_sat, data = SYCA_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
# 
# ## satq10temp
# mle_specs_satq10 <- specs(mm_name(type = "mle", GPP_fun = 'satlight', ER_fun = 'q10temp'))
# mm1_satq10 <- metab_mle(mle_specs_satq10, data = SYCA_met_full)
# 
# # identify negative values
...
#   facet_wrap(~model, scales = 'free_y')
# # 
# mm2_satq10 <- metab_mle(mle_specs_satq10, data = SYCA_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))
# 
# mm3_satq10 <- metab_mle(mle_specs_satq10, data = SYCA_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))
# 
# mm4_satq10 <- metab_mle(mle_specs_satq10, data = SYCA_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))
# 
# mm5_satq10 <- metab_mle(mle_specs_satq10, data = SYCA_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
# ####
# ####
# modList = ls()[grep("^mm\\d{1}.*", ls())] %>% purrr::map(~eval(as.symbol(.x)))
# save(modList, file = "./ignore/metab-models/mleModLists/SYCAmlemods.Rdata")
# 
# # model assessment ----
# rm(list = ls())
# source("./code/resources/01_load-packages.R")
# load(file = "./ignore/metab-models/mleModLists/SYCAmlemods.Rdata")
# modList_sub = unlist(lapply(modList, function(x) !is.data.frame(x)))
# modList = modList[modList_sub]
# 
...
# saveRDS(mm1_sat, "./ignore/metab-models/SYCA_full_mle.rds")
...
# metab_fun(specs = bayes_specs, data = SYCA_met_mod, data_daily = data_daily, info = info)
# 
# 
# 
# 
...
#   data = SYCA_met_mod,    # named list of data
...
# # 
# debugonce(metab)
# mm_bin <- metab(bayes_specs, data=SYCA_met_mod)
# 
# saveRDS(mm_bin, "./ignore/metab-models/SYCA_test.rds")