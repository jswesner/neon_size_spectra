...
# debugonce(metab_mle)
mm1 <- metab_mle(specs(mm_name(type = "mle")), data = CARI_met_full)
...
k600_mm2 %>%
  ...
  coord_trans('log10')
# 
mm3 <- metab_mle(mle_specs, data = CARI_met_full, data_daily = k600_mm2 %>% dplyr::select(-discharge.daily))
...
saveRDS(eval(as.name(topMod)), "./ignore/metab-models/CARI_full_mle.rds")
