rm(list = ls())
here::i_am("code/resources/metModelsMLE/CARI_metModel.R")
source(here::here("code/resources/01_load-packages.R"))
# 
rerun =FALSE
if(rerun){
tictoc::tic();run_metab_multimodel(siteCode = "CARI", metDf = NULL, return = FALSE, save = TRUE, parallel = FALSE);tictoc::toc()
}
siteCode = 'CARI'
load(file = here::here("ignore/metab-models/mleModLists/CARImlemods.Rdata"))
...
# 
# saveRDS(mm4, "./ignore/metab-models/CARI_full_mle.rds")

# # Getting 16 models should be 15
# rm(list = ls())
# source("./code/resources/01_load-packages.R")
# CARI_met_full = get_site_data(siteCode = "CARI") %>%
#   dplyr::select(-DO.pctsat)
# 
# discharge.daily = CARI_met_full %>%
...
#                           day_start = -1, day_end = 23), data_daily = k600_mm1)
# km4 <- metab_night(data = CARI_met_full )
# 
# # 
...
# # 
# mm2 <- metab_mle(mle_specs, data = CARI_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))
# 
# mm3 <- metab_mle(mle_specs, data = CARI_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))
# 
# mm4 <- metab_mle(mle_specs, data = CARI_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))
# 
# mm5 <- metab_mle(mle_specs, data = CARI_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
# ## 
# mle_specs_sat <- specs(mm_name(type = "mle", GPP_fun = 'satlight'))
# mm1_sat <- metab_mle(mle_specs_sat, data = CARI_met_full)
# 
...
#                               day_start = -1, day_end = 23), data_daily = k600_mm1_sat%>% select(-GPP.daily))
# 
# km4_sat <- metab_night(data = CARI_met_full)
# # 
...
#   facet_wrap(~model, scales = 'free_y')
# # 
# mm2_sat <- metab_mle(mle_specs_sat, data = CARI_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))
# 
# mm3_sat <- metab_mle(mle_specs_sat, data = CARI_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))
# 
# mm4_sat <- metab_mle(mle_specs_sat, data = CARI_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))
# 
# mm5_sat <- metab_mle(mle_specs_sat, data = CARI_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
# 
# ## satq10temp
# mle_specs_satq10 <- specs(mm_name(type = "mle", GPP_fun = 'satlight', ER_fun = 'q10temp'))
# mm1_satq10 <- metab_mle(mle_specs_satq10, data = CARI_met_full)
# 
# # identify negative values
...
#   facet_wrap(~model, scales = 'free_y')
# # 
# mm2_satq10 <- metab_mle(mle_specs_satq10, data = CARI_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))
# 
# mm3_satq10 <- metab_mle(mle_specs_satq10, data = CARI_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))
# 
# mm4_satq10 <- metab_mle(mle_specs_satq10, data = CARI_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))
# 
# mm5_satq10 <- metab_mle(mle_specs_satq10, data = CARI_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
# ####
# 
# modList = ls()[grep("^mm\\d{1}.*", ls())] %>% purrr::map(~eval(as.symbol(.x)))
# save(modList, file = "./ignore/metab-models/mleModLists/CARImlemods.Rdata")
# 
# # # model assessment ----
# # rm(list = ls())
# # source("./code/resources/01_load-packages.R")
# # load(file = "./ignore/metab-models/mleModLists/CARImlemods.Rdata")
# # modList_sub = unlist(lapply(modList, function(x) !is.data.frame(x)))
# # modList = modList[modList_sub]
# # 
...
# # 
# # saveRDS(mm3_sat, "./ignore/metab-models/CARI_full_mle.rds")
###
...
# metab_fun(specs = bayes_specs, data = CARI_met_mod, data_daily = data_daily, info = info)
# 
# 
# 
# 
...
#   data = CARI_met_mod,    # named list of data
...
# # 
# debugonce(metab)
# mm_bin <- metab(bayes_specs, data=CARI_met_mod)
# 
# saveRDS(mm_bin, "./ignore/metab-models/CARI_test.rds")