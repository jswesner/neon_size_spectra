# some limitations 
rm(list = ls())
here::i_am("code/resources/metModelsMLE/MCRA_metModel.R")
source(here::here("code/resources/01_load-packages.R"))

rerun = FALSE
if(rerun){
  tictoc::tic();run_metab_multimodel(siteCode = "MCRA", metDf = NULL, return = FALSE, save = TRUE, parallel = FALSE);tictoc::toc()
}

siteCode = 'MCRA'
load(file = here::here("ignore/metab-models/mleModLists/MCRAmlemods.Rdata"))
