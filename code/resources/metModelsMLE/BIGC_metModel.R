...
# 
mm2 <- metab_mle(mle_specs, data = BIGC_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))

mm3 <- metab_mle(mle_specs, data = BIGC_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))

mm4 <- metab_mle(mle_specs, data = BIGC_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))

mm5 <- metab_mle(mle_specs, data = BIGC_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
## 
mle_specs_sat <- specs(mm_name(type = "mle", GPP_fun = 'satlight'))
mm1_sat <- metab_mle(mle_specs_sat, data = BIGC_met_full)
...
k600_mm2_sat %>%
  ...
  facet_wrap(~model, scales = 'free_y')
# 
mm2_sat <- metab_mle(mle_specs_sat, data = BIGC_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))

mm3_sat <- metab_mle(mle_specs_sat, data = BIGC_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))

mm4_sat <- metab_mle(mle_specs_sat, data = BIGC_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))

mm5_sat <- metab_mle(mle_specs_sat, data = BIGC_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))

## satq10temp
mle_specs_satq10 <- specs(mm_name(type = "mle", GPP_fun = 'satlight', ER_fun = 'q10temp'))
mm1_satq10 <- metab_mle(mle_specs_satq10, data = BIGC_met_full)
...
k600_mm2_satq10 %>%
  ...
  facet_wrap(~model, scales = 'free_y')
# 
mm2_satq10 <- metab_mle(mle_specs_satq10, data = BIGC_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))

mm3_satq10 <- metab_mle(mle_specs_satq10, data = BIGC_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))

mm4_satq10 <- metab_mle(mle_specs_satq10, data = BIGC_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))

mm5_satq10 <- metab_mle(mle_specs_satq10, data = BIGC_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
####
####
