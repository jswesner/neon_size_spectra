rm(list = ls())
here::i_am("code/resources/metModelsMLE/BIGC_metModel.R")
source(here::here("code/resources/01_load-packages.R"))

rerun = FALSE
if(rerun){
tictoc::tic();run_metab_multimodel(siteCode = "BIGC", metDf = NULL, return = FALSE, save = TRUE, parallel = FALSE);tictoc::toc()
}

siteCode = 'BIGC'
load(file = here::here("ignore/metab-models/mleModLists/BIGCmlemods.Rdata"))
...
# 
# 
# ensembleIDList = ensembleMods$modelID %>%  unlist
# 
# extract_metab = function(mm= NULL,...){
#   modName = get_metab_info(mm, values = 'name')
#   
#  mm@metab_daily %>% 
#     as.data.frame %>% 
#     dplyr::select(date, "GPP") %>% 
#    dplyr::rename_with(~str_c(as.character(modName),"_",.), .cols = GPP) 
#    
# }
# DOPredDf = purrr::map(ensembleIDList, ~extract_metab(eval(as.symbol(.x)))) %>% 
#   reduce(left_join, by = 'date')
# 
# DOpred.xts = xts(DOPredDf, order.by= DOPredDf$date) 
# dygraph(DOpred.xts, main = "BIGC metab") %>% dyRangeSelector()
# 
# mm1PredDf = extract_metab(mm1)
# mm1Pred.xts = xts(mm1PredDf$mm1_GPP, order.by = mm1PredDf$date)
# dygraph(mm1Pred.xts, main = "mm1 Pred") %>% dyRangeSelector()
#####
# BIGC_met_full = get_site_data(siteCode = "BIGC") %>%
#   dplyr::select(-DO.pctsat)
# 
# BIGC_kGAM = readRDS("./ignore/site-gpp-data/BIGC_kGAM.rds")
# 
# discharge.daily = BIGC_met_full %>%
#   dplyr::mutate(date = as.Date(solar.time)) %>%
#   group_by(date) %>%
#   dplyr::summarise(discharge = mean(discharge, na.rm = TRUE),
#                    model = 'gam') 
# 
# discharge.daily$K600.daily = exp(predict(BIGC_kGAM, newdata = discharge.daily))
# 
# ## 
# ## 
# mle_specs <- specs(mm_name(type = "mle"))
# mm1 <- metab_mle(mle_specs, data = BIGC_met_full, info = list(name = "mm1", model = "raw"))
# 
...
#                           day_start = -1, day_end = 23), data_daily = k600_mm1 %>% select(-GPP.daily))
# km4 <- metab_night(data = BIGC_met_full)
# # 
...
# # 
# mm2 <- metab_mle(mle_specs, data = BIGC_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily),info = list(name = "mm2", model = "loess"))
# 
# mm3 <- metab_mle(mle_specs, data = BIGC_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily),info = list(name = "mm3", model = "lm"))
# 
# mm4 <- metab_mle(mle_specs, data = BIGC_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily),info = list(name = "mm4", model = "mean"))
# 
# mm5 <- metab_mle(mle_specs, data = BIGC_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily),info = list(name = "mm5", model = "night"))
# 
# mm6 <- metab_mle(mle_specs, data = BIGC_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'gam') %>% dplyr::select(date, K600.daily),info = list(name = "mm6", model = "gam"))
# #####
# mle_specs_sat <- specs(mm_name(type = "mle", GPP_fun = 'satlight'))
# mm1_sat <- metab_mle(mle_specs_sat, data = BIGC_met_full,info = list(name = "mm1_sat", model = "raw"))
# 
...
#   facet_wrap(~model, scales = 'free_y')
# # 
# mm2_sat <- metab_mle(mle_specs_sat, data = BIGC_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily),info = list(name = "mm2_sat", model = "loess"))
# 
# mm3_sat <- metab_mle(mle_specs_sat, data = BIGC_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily),info = list(name = "mm3_sat", model = "lm"))
# 
# mm4_sat <- metab_mle(mle_specs_sat, data = BIGC_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily),info = list(name = "mm4_sat", model = "mean"))
# 
# mm5_sat <- metab_mle(mle_specs_sat, data = BIGC_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily),info = list(name = "mm5_sat", model = "night"))
# 
# mm6_sat <- metab_mle(mle_specs_sat, data = BIGC_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'gam') %>% dplyr::select(date, K600.daily),info = list(name = "mm6_sat", model = "gam"))
# 
# ## satq10temp
# mle_specs_satq10 <- specs(mm_name(type = "mle", GPP_fun = 'satlight', ER_fun = 'q10temp'))
# mm1_satq10 <- metab_mle(mle_specs_satq10, data = BIGC_met_full,info = list(name = "mm1_satq10", model = "raw"))
# 
# # identify negative values
...
#   facet_wrap(~model, scales = 'free_y')
# # 
# mm2_satq10 <- metab_mle(mle_specs_satq10, data = BIGC_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily),info = list(name = "mm2_satq10", model = "loess"))
# 
# mm3_satq10 <- metab_mle(mle_specs_satq10, data = BIGC_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily), info = list(name = "mm3_satq10", model = "lm"))
# 
# mm4_satq10 <- metab_mle(mle_specs_satq10, data = BIGC_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily),info = list(name = "mm4_satq10", model = "mean"))
# 
# mm5_satq10 <- metab_mle(mle_specs_satq10, data = BIGC_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily),info = list(name = "mm5_satq10", model = "night"))
# 
# mm6_satq10 <- metab_mle(mle_specs_satq10, data = BIGC_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'gam') %>% dplyr::select(date, K600.daily), info = list(name = "mm6_satq10", model = "gam"))
# 
# 
# ####
# modList = ls()[grep("^mm\\d{1}.*", ls())] %>% purrr::map(~eval(as.symbol(.x))) 
# save(modList, file = "./ignore/metab-models/mleModLists/BIGCmlemods.rds")
# 
# siteCode = 'BIGC'
# rmarkdown::render(here::here("./code/resources/metModelsMLE/MLE-model-report-template.Rmd"),
#                   output_format = "pdf_document",
...
# 
# saveRDS(mm3, "./ignore/metab-models/BIGC_full_mle.rds")
...
# metab_fun(specs = bayes_specs, data = BIGC_met_mod, data_daily = data_daily, info = info)
# 
# 
# 
# 
...
#   data = BIGC_met_mod,    # named list of data
...
# # 
# debugonce(metab)
# mm_bin <- metab(bayes_specs, data=BIGC_met_mod)
# 
# saveRDS(mm_bin, "./ignore/metab-models/BIGC_test.rds")