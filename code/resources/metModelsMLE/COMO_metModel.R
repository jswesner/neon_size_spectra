rm(list = ls())
here::i_am("code/resources/metModelsMLE/COMO_metModel.R")
source(here::here("code/resources/01_load-packages.R"))

rerun = FALSE
if(rerun){
  tictoc::tic();run_metab_multimodel(siteCode = "COMO", metDf = NULL, return = FALSE, save = TRUE, parallel = FALSE);tictoc::toc()
}

siteCode = 'COMO'
load(file = here::here("ignore/metab-models/mleModLists/COMOmlemods.Rdata"))
