rm(list = ls())
here::i_am("code/resources/metModelsMLE/KING_metModel.R")
source(here::here("code/resources/01_load-packages.R"))
# 
rerun = FALSE
if(rerun){
  tictoc::tic();run_metab_multimodel(siteCode = "KING", metDf = NULL, return = FALSE, save = TRUE, parallel = FALSE);tictoc::toc()
}
siteCode = 'KING'
load(file = here::here("ignore/metab-models/mleModLists/KINGmlemods.Rdata"))
...
#                           day_start = -1, day_end = 23), data_daily = k600_mm1)
# km4 <- metab_night(data = KING_met_full)
# # 
...
# # 
# mm2 <- metab_mle(mle_specs, data = KING_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))
# 
# mm3 <- metab_mle(mle_specs, data = KING_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))
# 
# mm4 <- metab_mle(mle_specs, data = KING_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))
# 
# mm5 <- metab_mle(mle_specs, data = KING_met_full, data_daily = k600_mm2 %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
# ## 
# mle_specs_sat <- specs(mm_name(type = "mle", GPP_fun = 'satlight'))
# mm1_sat <- metab_mle(mle_specs_sat, data = KING_met_full)
# 
...
#   facet_wrap(~model, scales = 'free_y')
# # 
# mm2_sat <- metab_mle(mle_specs_sat, data = KING_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))
# 
# mm3_sat <- metab_mle(mle_specs_sat, data = KING_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))
# 
# mm4_sat <- metab_mle(mle_specs_sat, data = KING_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))
# 
# mm5_sat <- metab_mle(mle_specs_sat, data = KING_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
# 
# ## satq10temp
# mle_specs_satq10 <- specs(mm_name(type = "mle", GPP_fun = 'satlight', ER_fun = 'q10temp'))
# mm1_satq10 <- metab_mle(mle_specs_satq10, data = KING_met_full)
# 
# # identify negative values
...
#   facet_wrap(~model, scales = 'free_y')
# # 
# mm2_satq10 <- metab_mle(mle_specs_satq10, data = KING_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'loess') %>% dplyr::select(date, K600.daily))
# 
# mm3_satq10 <- metab_mle(mle_specs_satq10, data = KING_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'lm') %>% dplyr::select(date, K600.daily))
# 
# mm4_satq10 <- metab_mle(mle_specs_satq10, data = KING_met_full, data_daily = k600_mm2_satq10 %>% dplyr::filter(model == 'mean') %>% dplyr::select(date, K600.daily))
# 
# mm5_satq10 <- metab_mle(mle_specs_satq10, data = KING_met_full, data_daily = k600_mm2_sat %>% dplyr::filter(model == 'night') %>% dplyr::select(date, K600.daily))
# ####
# ####
# modList = ls()[grep("^mm\\d{1}.*", ls())] %>% purrr::map(~eval(as.symbol(.x)))
# save(modList, file = "./ignore/metab-models/mleModLists/KINGmlemods.rds")
# 
# # model assessment ----
# rm(list = ls())
# source("./code/resources/01_load-packages.R")
# load(file = "./ignore/metab-models/mleModLists/KINGmlemods.Rdata")
...
# metab_fun(specs = bayes_specs, data = KING_met_mod, data_daily = data_daily, info = info)
# 
# 
# 
# 
...
#   data = KING_met_mod,    # named list of data
...
# # 
# debugonce(metab)
# mm_bin <- metab(bayes_specs, data=KING_met_mod)
# 
# saveRDS(mm_bin, "./ignore/metab-models/KING_test.rds")