...
get_sample_mat_lambdas <- function(model = NA, data = NA, grp1_contains = "alpha_raw_year",
                                   grp2_contains = "alpha_raw_site",
                                   grp3_contains = "alpha_raw_sample"){
...
get_sample_om_lambdas <- function(model = NA, data = NA, grp1_contains = "alpha_raw_year",
                                   grp2_contains = "alpha_raw_site",
                                   grp3_contains = "alpha_raw_sample"){
  ...
  sample_pivot = as_draws_df(model) %>% 
    ...
    mutate(sample_int = parse_number(sample_int)) %>% 
    left_join(data %>% ungroup %>% distinct(sample_int, year_int, site_int, 
                                            log_om_s, year, site_id, mean_om)) %>% 
    left_join(year_pivot, by = c(".iteration",".draw","year_int")) %>% 
    left_join(site_pivot, by = c(".iteration",".draw","site_int")) 
  
  posts_long_parsed = sample_pivot %>%
    mutate(lambda = a + beta_om*log_om_s + 
             # beta_gpp*log_gpp_s + 
             # beta_om*log_om_s +
             # beta_gpp_om*log_gpp_s*log_om_s + 
             # beta_gpp_mat*log_gpp_s*mat_s + 
...
get_sample_gpp_lambdas <- function(model = NA, data = NA, grp1_contains = "alpha_raw_year",
                                  grp2_contains = "alpha_raw_site",
                                  grp3_contains = "alpha_raw_sample"){
  ...
  sample_pivot = as_draws_df(model) %>% 
    ...
    mutate(sample_int = parse_number(sample_int)) %>% 
    left_join(data %>% ungroup %>% distinct(sample_int, year_int, site_int, 
                                            log_gpp_s, year, site_id, gpp)) %>% 
    left_join(year_pivot, by = c(".iteration",".draw","year_int")) %>% 
    left_join(site_pivot, by = c(".iteration",".draw","site_int")) 
  
  posts_long_parsed = sample_pivot %>%
    mutate(lambda = a + beta_gpp*log_gpp_s + 
             # beta_gpp*log_gpp_s + 
             # beta_om*log_om_s +
             # beta_gpp_om*log_gpp_s*log_om_s + 
             # beta_gpp_mat*log_gpp_s*mat_s + 
