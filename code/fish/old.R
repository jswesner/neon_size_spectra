...
perfish <- fish$fsh_perFish %>% as_tibble() %>% clean_names() %>% 
  ...
  ungroup() %>% 
  anti_join(fixed_ids) %>% 
  mutate(reach_date_site = paste(reach, date, site_id, sep = "_"))

# bulk count of additional fish after the first 50 in fsh_perFish. Doesn't include bulk counts of zero.
...
counts_by_reachdatesite <- bulk_count %>% select(reach_date_site, pass_number, date, taxon_id, bulk_fish_count) %>% 
  group_by(reach_date_site, taxon_id) %>% 
  summarize(n_bulk = sum(bulk_fish_count))

perfish_plus_bulk <- perfish %>% 
  select(reach_date_site, pass_number, date, taxon_id, fish_total_length) %>% 
  group_by(reach_date_site, taxon_id) %>% 
  tally(name = "n_perfish") %>% 
  left_join(counts_by_reachdatesite) %>% 
  replace_na(list(n_bulk = 0)) %>% 
  mutate(perfishmore50 = case_when(n_perfish > 55 ~ 1,
                                   TRUE ~ 0))


perfish_plus_bulk  %>% 
  ggplot(aes(x = n_perfish, y = n_bulk, color = perfishmore50)) + 
  geom_point(alpha = 0.2) 


# there are 23/1947 collections that measured more than 55 fish
perfish_plus_bulk %>%
  ungroup() %>% 
  group_by(perfishmore50) %>% 
  tally()

perfish_withtotals = perfish %>% 
  left_join(perfish_plus_bulk %>% distinct(reach_date_site, taxon_id, n_perfish, n_bulk, perfishmore50)) %>% 
  mutate(total_fish = n_perfish + n_bulk)

# Of the 29,988 records of length, 1759 were from more than 55 fish.
perfish_withtotals %>%
  group_by(perfishmore50) %>% 
  tally()
...
# If 400 fish caugh, then sample 400 lenghts (with replacement) from the 50 length measurements

###### NEED TO FIX THIS SO THAT IT IS PER PASS !!!! (2022-08-10)

fish_lengths = perfish_withtotals %>% 
  mutate(n_replicate = total_fish/n_perfish) %>% # factor to replicate by
  slice(rep(1:n(), n_replicate))
