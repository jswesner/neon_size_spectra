...
fish$fsh_bulkCount %>% 
  ggplot(aes(x = passNumber, y = bulkFishCount)) + 
  geom_point() + 
  geom_jitter(width = 0.2, height = 0)

stream_fish_with_passes %>% 
  ggplot(aes(x = pass, y = total_fish)) + 
  geom_point()
...
ubms_data = stream_fish_with_passes %>% 
  ...
  ungroup() %>% 
  mutate(site_int = as.factor(1:nrow(.)))
...
fm1 <- stan_multinomPois(~ site_int ~ site_int, ovenFrame,
                         chains = 1, iter = 300)



fm1 = update(fm1, iter = 1000, chains = 4)
saveRDS(fm1, file = "code/fish/models/stan/fm1.rds")

library(tidybayes)
library(brms)
fm_draws = as_draws_df(fm1@stanfit)
# abundance
...
bayes_freq = draws_long %>% 
  ...
  bind_rows(freq_only %>% mutate(model = "3-pass, FSA package (Frequentist)") %>% 
              ...
              rename(abundance = estimate, 
                     ...
                     .upper= upper,
                     site_int = site)) %>% 
  left_join(sites %>% rename(site_int = site)) %>% 
...
bayes_freq %>% 
  ggplot(aes(y = reorder(site_id, abundance), x = abundance, color = model)) + 
...
all_three = bayes_freq %>% 
  bind_rows(p_first_pois %>% select(site_int, .epred, .lower, .upper, model, sample, site_id) %>% rename(abundance = .epred))

method_comparison = all_three %>%
  distinct(site_int, model, sample, site_id, .keep_all = T) %>% 
  ggplot(aes(y = reorder(site_int, abundance), color = model)) + 
  geom_point(aes(x = abundance)) + 
  # geom_linerange(aes(xmin = .lower, xmax = .upper)) +
  scale_x_log10() +
...
ggsave(method_comparison, file = "code/fish/models/test_data/method_comparison.jpg",
       width = 5, height = 5)
saveRDS(method_comparison, file = "code/fish/models/test_data/method_comparison.rds")

regress_three_one = all_three %>% 
  distinct(site_int, model, sample, site_id, .keep_all = T) %>% 
  select(site_int, site_id, abundance, model) %>%
  ...
  geom_abline() + 
  labs(title = "Single-pass accruately predicts total populations\nwhen corrected for detection probabilities") +
  theme_default()
saveRDS(regress_three_one, file = "code/fish/models/test_data/regress_three_one.rds")


# Compare richness ---------------------------------------------------------

three_pass_richness = stream_fish_with_passes %>% ungroup %>% 
  distinct(site_id, date, taxon_id) %>% 
  group_by(site_id, date) %>% 
  tally() %>% 
  mutate(data = "three pass")

one_pass_richness = stream_fish_with_passes %>% ungroup %>% 
  filter(pass == 1) %>% 
  distinct(site_id, date, taxon_id) %>% 
  group_by(site_id, date) %>% 
  tally() %>% 
  mutate(data = "one pass")

three_and_one_pass_richness = bind_rows(three_pass_richness, one_pass_richness) %>% 
  group_by(site_date = paste0(site_id, date),
           site_int = as.integer(as.factor(site_date)))



methods_compare_richness = three_and_one_pass_richness %>% 
  select(data, n, site_int) %>% 
  pivot_wider(names_from = data, values_from = n) %>% 
  ggplot(aes(x = `one pass`, y = `three pass`)) +
  geom_point() +
  geom_abline() +
  labs(title = "One pass collects exactly the same # species as three pass",
       x = "# species collected in first pass",
       y = "# species collected in three passes") +
  theme_default()

ggsave(methods_compare_richness, file = "code/fish/models/test_data/methods_compare_richness.jpg",
       width = 5, height = 5)

saveRDS(methods_compare_richness, file = "code/fish/models/test_data/methods_compare_richness.rds")
