...
three_pass_data_taxon = read_csv("data/raw_data/fish/three_pass_data_taxon.csv") %>%   # restrict to fixed reaches only
  filter(reach_id %in% fixed_reaches) # made with get_pass_data.R

# fit multinomial poisson (three pass depletion model) -----------------------------------------------
# wrangle
three_pass_data_taxon_wide = three_pass_data_taxon %>% 
...
three_pass_taxon_matrix = three_pass_data_taxon_wide %>% 
  select(`1`,`2`,`3`) %>% 
  as.matrix()

# assign covariates (for ubms)
three_pass_taxon_frame <- unmarkedFrameMPois(three_pass_taxon_matrix,
                                siteCovs=as.data.frame(three_pass_data_taxon_wide %>% select(site_int)),
                                type = "removal")

saveRDS(three_pass_taxon_frame, file = "data/raw_data/fish/three_pass_taxon_frame.rds")

# fit model
# this estimates population size and capture efficiency for each reach_id (called site_int here)
three_pass_taxon_model = stan_multinomPois(formula = ~site_int ~ site_int, 
                                     data = three_pass_taxon_frame, 
                                     chains = 1, iter = 10)

saveRDS(three_pass_taxon_model, file = "data/raw_data/fish/three_pass_taxon_model.rds")

three_pass_taxon_model = update(readRDS(file = "data/raw_data/fish/three_pass_taxon_model.rds"),
                                iter = 2000, chains = 4)

three_pass_taxon_model = readRDS(file = "data/raw_data/fish/three_pass_taxon_model.rds")
...
###########################################
###########################################
#2023-02-10 - Jeff the code below is not updated. Change it to include taxa before running
###########################################
###########################################
...
site_capture_probs = capture_prob_posts %>% 
  ...
  left_join(three_pass_data %>% ungroup %>% # add identifiers
              mutate(site_int = parse_number(as.character(site_int))) %>% 
              distinct(site_int, site_id, reach_id, date)) %>% 
...
fish_total_abundance_poisson = brm(total_fish ~ reach_id + offset(median_prob),
                             ...
                             chains = 1, iter = 1000,
                             file = "code/models/model-fits/fish_total-abundance-poisson.rds",
...
all_population_estimates %>% 
  ...
  ggplot(aes(x = pop_singlepass, y = pop_threepass)) + 
  geom_point(size = 1) +
  geom_errorbarh(aes(xmin = .lower_singlepass, xmax = .upper_singlepass), alpha = 0.2) + 
  geom_errorbar(aes(ymin = .lower_threepass, ymax = .upper_threepass), alpha = 0.2) +
  geom_abline() + 
  # scale_x_log10() +
  # scale_y_log10() +
  theme_default() +
