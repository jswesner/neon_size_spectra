...
#                                      chains = 4, iter = 1000)
# 
# saveRDS(three_pass_model, file = "data/raw_data/fish/three_pass_model.rds")
# ...or just load the fit model here
...
capture_prob_posts = as_draws_df(three_pass_model@stanfit) %>% 
  ...
  left_join(three_pass_data_wide %>% ungroup %>% # add identifiers
...
# summarize capture probabilities 
site_year_capture_probs = capture_prob_posts %>% 
  group_by(site_id, year) %>% 
  median_qi(median_prob = inv_logit_scaled(value)) %>% 
...
fish_total_abundance_poisson = readRDS("code/models/model-fits/fish_total-abundance-poisson.rds")
...
site_year_capture_probs %>% 
  # group_by(site_id) %>% 
  # mutate(order = as.integer(as.factor(median(median_prob))),
  #        order_site = paste(order, site_id, sep = "_")) %>% 
  #        site_id = as.character(site_id),
  #        site_id = fct_relevel(site_id, order)) %>%  
  ggplot(aes(x = year, y = median_prob, color = site_id)) + 
