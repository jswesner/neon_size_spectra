...
dat = neon_sizes_2016_2021 %>% filter(sample_int == 12)
...
sizeSpectra::MLE.plot(x = dat$dw*dat$no_m2, b = summary(fit_interaction)$c_summary[[1]])

# saveRDS(fit_interaction, file = paste0("models/stan_gppxtempxom",Sys.Date(),".rds"))

# redo with simulated data ---------------------------------------------------------

dat = neon_sizes_2016_2021 %>% filter(sample_int == 12) %>% 
  sample_n(size = 300, weight = no_m2, replace = T) %>% 
  group_by(dw, mat_s, log_gpp_s, log_om_s, year_int, site_int, sample_int, xmin, xmax) %>% 
  tally(name = "no_m2")
...
fit_sim = sampling(object = stan_spectra_single,
                           data = stan_data_interaction,
                           iter = 2000, chains = 2)



sizeSpectra::MLE.plot(x = dat$dw, b = summary(fit_sim)$c_summary[[1]])
